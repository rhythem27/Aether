import pytest
from backend.db.neo4j import get_neo4j_driver, init_neo4j_schema, bulk_write_nodes_and_relationships
...
async def test_neo4j_schema_and_in_memory_driver():
    ...
    nodes = [
        {"id": "company_apple_inc", "name": "Apple Inc.", "label": "Company", "properties": {"ticker": "AAPL"}},
        {"id": "company_beats", "name": "Beats Electronics", "label": "Company", "properties": {}}
    ]
    relationships = [
        {"source_id": "company_apple_inc", "target_id": "company_beats", "rel_type": "ACQUIRED", "properties": {"year": 2014}}
    ]
    
    written = await bulk_write_nodes_and_relationships(nodes, relationships, driver=driver)
    assert written == 3
...
def test_entity_resolver_canonicalization_and_deduplication():
    ...
    raw_data = ExtractedGraphData(
        nodes=[
            ...
            EntityNode(id="c2", name="AAPL", label=EntityType.COMPANY, properties={"sector": "Tech"}),
            EntityNode(id="c3", name="Tesla Inc.", label=EntityType.COMPANY)
        ],
        relationships=[
            RelationshipTriple(source_id="company_apple", target_id="company_tesla", rel_type=RelationType.COMPETES_WITH),
            RelationshipTriple(source_id="company_apple", target_id="company_tesla", rel_type=RelationType.COMPETES_WITH)  # Duplicate
        ]
    )
...
async def test_financial_graphrag_extraction_and_indexing():
    ...
    assert RelationType.INVESTED_IN in rel_types or RelationType.ACQUIRED in rel_types or RelationType.COMPETES_WITH in rel_types
