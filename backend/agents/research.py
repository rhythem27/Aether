...
async def research_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        mcp_tools = mcp_client.get_tools_for_servers(["sec_edgar", "crunchbase", "newsapi"])
        ...
        filings = await mcp_client.execute_tool_with_retry(search_filings_fn, ticker=ticker, form_type="10-K", limit=3)

        financial_extracts = {}
        if filings and isinstance(filings, list) and len(filings) > 0:
            filing_url = filings[0].get("file_url", filings[0].get("sample_filing_url", ""))
            if filing_url:
                financial_extracts = await mcp_client.execute_tool_with_retry(extract_financials_fn, filing_url=filing_url)

        # 2. Crunchbase Private Market & M&A Data
        funding_fn = mcp_tools["get_funding_rounds"]
        funding_rounds = await mcp_client.execute_tool_with_retry(funding_fn, company_id=ticker.lower())

        # 3. NewsAPI Sentiment & Market News
        news_fn = mcp_tools["get_recent_news"]
        sentiment_fn = mcp_tools["analyze_news_sentiment"]
        
        recent_news = await mcp_client.execute_tool_with_retry(news_fn, query=company_name, limit=3)
        news_sentiment = await mcp_client.execute_tool_with_retry(sentiment_fn, articles=recent_news)
