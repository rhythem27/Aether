...
async def init_qdrant_collection(client: Optional[AsyncQdrantClient] = None, collection_name: str = FINANCIAL_COLLECTION_NAME):
        ...
        if collection_name not in existing_names:
            ...
            for field_name, field_schema in payload_fields:
                try:
                    ...
                    logger.warning("payload_index_creation_warn", field=field_name, error=str(idx_err))
        else:
            logger.info("qdrant_collection_exists", collection=collection_name)
    except Exception as e:
        logger.error("qdrant_init_collection_failed", collection=collection_name, error=str(e))
        raise
