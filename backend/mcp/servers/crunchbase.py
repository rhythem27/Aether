...
async def get_funding_rounds(company_id: str) -> List[Dict[str, Any]]:
    ...
    headers = {"Authorization": f"Bearer {settings.CRUNCHBASE_KEY}"} if hasattr(settings, "CRUNCHBASE_KEY") else {}
...
async def get_investors(company_id: str) -> List[Dict[str, Any]]:
    ...
    return [
        {"investor_name": "Sequoia Capital", "type": "Venture Capital", "lead_rounds": 2, "country": "USA"},
        {"investor_name": "Accel Partners", "type": "Venture Capital", "lead_rounds": 1, "country": "USA"},
        {"investor_name": "Andreessen Horowitz", "type": "Venture Capital", "lead_rounds": 0, "country": "USA"}
    ]
