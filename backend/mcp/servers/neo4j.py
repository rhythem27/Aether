...
async def query_entity_subgraph(entity_name: str, max_depth: int = 2) -> Dict[str, Any]:
    ...
    logger.info("neo4j_mcp_query_entity_subgraph", entity_name=entity_name, max_depth=max_depth)
...
async def find_paths_between(entity_a: str, entity_b: str) -> List[Dict[str, Any]]:
    ...
    try:
        async with driver.session() as session:
            res = await session.run(cypher, parameters={"entity_a": entity_a, "entity_b": entity_b})
            records = await res.data()
            return records
    except Exception as e:
        logger.error("neo4j_find_paths_failed", entity_a=entity_a, entity_b=entity_b, error=str(e))
        return []
...
async def execute_cypher(read_query: str) -> List[Dict[str, Any]]:
    ...
    for kw in forbidden:
        if re.search(rf"\b{kw}\b", query_upper):
            raise ValueError(f"Forbidden write operation '{kw}' detected in read-only Cypher query.")

    logger.info("neo4j_mcp_execute_cypher", query=read_query)
    driver = await _get_active_driver()

    if isinstance(driver, InMemoryNeo4jDriver):
        return [{"status": "success", "mock_records": list(driver.db["nodes"].values())[:5]}]
