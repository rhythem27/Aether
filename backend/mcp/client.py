...
class MCPClientManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        if os.path.exists(self.config_path):
            try:
                ...
                logger.warning("mcp_config_load_failed", path=self.config_path, error=str(e))

        # Default fallback server registry
        return {
            "mcpServers": {
                "sec_edgar": {"command": "python", "args": ["-m", "backend.mcp.servers.sec_edgar"]},
                "crunchbase": {"command": "python", "args": ["-m", "backend.mcp.servers.crunchbase"]},
                "newsapi": {"command": "python", "args": ["-m", "backend.mcp.servers.newsapi"]},
                "neo4j_graph": {"command": "python", "args": ["-m", "backend.mcp.servers.neo4j"]}
            }
        }
    ...
    async def execute_tool_with_retry(self, tool_func: Callable, *args: Any, **kwargs: Any) -> Any:
    ...
    def get_tools_for_servers(self, server_names: Optional[List[str]] = None) -> Dict[str, Callable]:
