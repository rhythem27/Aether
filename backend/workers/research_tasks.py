...
from backend.agents.state import BaseMessage
...
async def execute_research_job_async(job_id: str, target_company: str):
    ...
    JobManager.update_job_status(job_id, status="researching", progress_pct=15.0, current_agent="research_agent")
    JobManager.add_activity(job_id, AgentActivity(
    ...
    try:
        ...
        initial_state: AgentState = {
            "messages": [HumanMessage(content=f"Perform deep dive financial research on {target_company}")],
        ...
        async for output in app.astream(initial_state):
            for node_name, node_state in output.items():
                ...
                if node_name == "research_agent":
                    JobManager.update_job_status(job_id, status="analyzing", progress_pct=35.0, current_agent="analysis_agent")
                    JobManager.add_activity(job_id, AgentActivity(
                        agent_name="research_agent",
                    ...
                    JobManager.update_job_status(job_id, status="verifying", progress_pct=60.0, current_agent="verify_agent")
                    JobManager.add_activity(job_id, AgentActivity(
                    ...
                    JobManager.update_job_status(job_id, status="reporting", progress_pct=80.0, current_agent="report_agent")
                    JobManager.add_activity(job_id, AgentActivity(
                    ...
                    JobManager.update_job_status(job_id, status="complete", progress_pct=100.0, current_agent="report_agent", result=report_res)
                    JobManager.add_activity(job_id, AgentActivity(
        ...
        JobManager.update_job_status(job_id, status="error", progress_pct=100.0, error_message=str(e))
        JobManager.add_activity(job_id, AgentActivity(
